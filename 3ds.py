...
valid_front_serial_headers = ["LNA"]

print(f"No-Intro 3DS XML Generator {program_version} by kaos\n")
...
try:
    ...
    while file_path is None:
        file_path = input("Please enter a file path to use (must end in .3ds). ")
...
dat_path = ""
dat_loaded = constants['use_3ds_dat']
if dat_loaded is True:
    dat_path = constants['3ds_dat_path']
    # Check if it's actually there
    if not os.path.isfile(dat_path):
        print(f"constants.toml tried loading dat file {dat_path} but no file was found. Defaulting to disabled dat usage.")
        dat_loaded = False

n3ds_dat_path = ""
n3ds_dat_loaded = constants['use_n3ds_dat']
if n3ds_dat_loaded is True:
    n3ds_dat_path = constants['n3ds_dat_path']
    # Check if it's actually there
    if not os.path.isfile(n3ds_dat_path):
        print(f"constants.toml tried loading dat file {n3ds_dat_path} but no file was found. Defaulting to disabled dat usage.")
        n3ds_dat_loaded = False

# Extract what we can from the GM9 log.
gm9_output_lines = gm9_output.split('\n')
try:
    title_id = gm9_output_lines[0].split('Title ID     : ')[1]
    internal_serial = gm9_output_lines[1].split('Product Code : ')[1]
    revision = gm9_output_lines[2].split('Revision     : ')[1]
    cart_id = gm9_output_lines[3].split('Cart ID      : ')[1]
    platform = gm9_output_lines[4].split('Platform     : ')[1]  # GM9 seems to lie here. The platform is 'O3DS' even if it's a N3DS cart.
    ...
    if gm9_output_lines[7][0:15] == 'Padding Byte : ':
        # I think this is a GM9 2.2.0 thing.
        padding_byte = int(f"0x{gm9_output_lines[7][15:]}", 16)
    ...
    input(f"Error handling GM9 log: {e}. \nPress enter to exit.")
    sys.exit(1)

# If it's a 4GB file, check if it's been properly re-padded.
size_on_disk = os.path.getsize(file_path)
if size_on_disk == 4294967295:
    input(f".3ds file should be 4GB in size, but GM9 does not dump the last byte for 4GB files. \nPlease add hex {hex(padding_byte)} to the end of the .3ds file in a hex editor. \nPress enter to exit.")
    sys.exit(1)

# Get encrypted hashes using built-in python functionality.
...
with open(file_path, 'rb') as f:
    ...
    enc_crc32 = ("%08X" % (temp_crc32 & 0xFFFFFFFF)).lower()
    enc_md5 = md5_func.hexdigest()
    enc_sha1 = sha1_func.hexdigest()
    enc_sha256 = sha256_func.hexdigest()
...
#   3DS files are up to 4GB in size, so this may take a bit. (Warn user.)
...
print("Generating decrypted hashes.")
subprocess.run(f'NDecrypt.exe d -h "{temp_loc}"', stdout=subprocess.DEVNULL)
with open(hashfile_loc) as dec_hashfile:
    file_size = dec_hashfile.readline().split(': ')[1][:-1]
    dec_crc32 = dec_hashfile.readline().split(': ')[1][:-1]
    dec_md5 = dec_hashfile.readline().split(': ')[1][:-1]
    dec_sha1 = dec_hashfile.readline().split(': ')[1][:-1]
    dec_sha256 = dec_hashfile.readline().split(': ')[1][:-1]
...
# Determine which No-Intro DAT file to use.
if internal_serial[0:3] == 'KTR':
    dat_loaded = True
    dat_path = n3ds_dat_path
    n3ds_enabled = True
# otherwise, default to 3ds dat

# Check the loaded No-Intro DAT file for a match if enabled.
if dat_loaded:
    tree = ElementTree.parse(dat_path)
    root = tree.getroot()
    ...
    if no_intro_name is not None:
        ...
        try:
            ...
            # TODO: Handle special tags. It's too annoying to bother right now, and there's no 'new 3ds enhanced' like there is for DS.
        except IndexError:
            pass
        ...
        print("\nNo match found in 3DS/N3DS dat (could be bad dump or new dump, check online DAT info). Please enter data manually.")
        print(f"\nEncrypted hashes: \nCRC-32: {enc_crc32}\nMD5: {enc_md5}\nSHA-1: {enc_sha1}\nSHA-256: {enc_sha256}")
...
back_serial = input(f"Enter back serial (starts with {internal_serial[6:]}): ")
...
archive.setAttribute('gameid1',title_id)
...
details.setAttribute('originalformat', 'Encrypted')
...
decrypted_file.setAttribute('extension', '3ds')
...
encrypted_file.setAttribute('extension', '3ds')
...
if n3ds_enabled:
    print("This is a N3DS game cartridge. Please make sure to upload submissions as N3DS on No-Intro!")
